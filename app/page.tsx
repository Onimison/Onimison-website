    ...
    <div className="w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10 w-full max-w-7xl mx-auto">
          ...
          <div className="relative h-[60vh] hidden lg:block">
            <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
              <span className="font-bebas text-[15rem] -rotate-90 tracking-tighter select-none">
                ONIMISON
              </span>
            </div>
            ...
            <ScrollReveal
              delay={0.5}
              className="absolute top-1/4 left-1/4 w-32 h-48 bg-rust mix-blend-screen"
            ></ScrollReveal>
            <ScrollReveal
              delay={0.6}
              className="absolute top-1/3 right-1/4 w-40 h-40 border-2 border-off-white/30"
            ></ScrollReveal>
            <ScrollReveal
              delay={0.7}
              className="absolute bottom-1/4 left-1/3 w-1 h-64 bg-muted-grey/50"
            ></ScrollReveal>
            <ScrollReveal
              delay={0.8}
              className="absolute bottom-1/3 right-1/3 w-24 h-24 bg-[#d4af37]/80 mix-blend-screen"
            ></ScrollReveal>
          </div>
        </div>
      ...
      <section className="bg-rust py-4 overflow-hidden whitespace-nowrap flex items-center">
        <div className="animate-marquee flex space-x-8 items-center">
          {[...Array(2)].map((_, i) => (
            <div
              key={i}
              className="flex space-x-8 items-center font-bebas text-4xl tracking-wider text-bg-dark"
            >
      ...
      <section id="about" className="py-32 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div className="space-y-8">
            ...
            <ScrollReveal delay={0.1}>
              <div className="font-space-mono text-sm leading-relaxed text-off-white/80 space-y-6">
                <p>
                  It started with Photoshop. Then it bled into frontend
                  development because I wanted the things I designed to actually
                  exist. Then came storytelling, because code couldn&apos;t hold
                  everything I needed to say.
                </p>
                <p>
                  I co-founded 10.8.8 Africa, built the brand, and led Purple
                  University. I spent years in the crypto trenches — made real
                  money, lost it, learned how systems actually work when the
                  hype dies.
                </p>
                <p>
                  Now, I build digital infrastructure for NIGCOMSAT and take on
                  freelance projects that require more than just a template. I
                  don&apos;t fit neatly into a box, and I stopped trying to.
                </p>
              </div>
            </ScrollReveal>
          </div>
          ...
          <div className="relative pl-8 border-l border-rust/30 space-y-12">
            {[
              {
                title: "NIGCOMSAT + Freelance",
                date: "2024–Present",
                desc: "Creating contents for social media and playing around with AI systems.",
              },
              {
                title: "Crypto/Degen Era",
                date: "2022–2024",
                desc: "Made real money. Lost it. Learned the mechanics of hype and value.",
              },
              {
                title: "Frontend Developer at Paritie Innovation Hub",
                date: "2023–2024",
                desc: "Built functional frontend for in house projects and i taught JavaScript to a bunch of interns.",
              },
              {
                title: "Purple University Founder",
                date: "2020–2022",
                desc: "Led community, content, and education initiatives.",
              },
              {
                title: "10.8.8 Africa Co-founder",
                date: "2021–Present",
                desc: "Built the brand, the first site, and the structure.",
              },
            ].map((item, i) => (
              <ScrollReveal key={i} delay={i * 0.1} className="relative">
                <div className="absolute -left-[37px] top-1 w-2 h-2 rounded-full bg-rust"></div>
                <span className="font-space-mono text-xs text-rust mb-2 block">
                  {item.date}
                </span>
                <h3 className="font-dm-serif text-2xl mb-2">{item.title}</h3>
                <p className="font-space-mono text-xs text-muted-grey">
                  {item.desc}
                </p>
              </ScrollReveal>
            ))}
      ...
      <section id="blog" className="bg-cream text-dark-ink py-32 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <span className="font-space-mono text-xs uppercase tracking-widest text-rust mb-12 block">
              03 — Blog
            </span>
          </ScrollReveal>
          ...
          <div className="border-t border-dark-ink/20">
            {blogPosts.slice(0, 4).map((post, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group flex flex-col md:flex-row items-start md:items-center justify-between py-12 border-b border-dark-ink/20 hover:bg-dark-ink/5 transition-colors px-4 -mx-4 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-rust focus-visible:outline-none rounded-sm"
                >
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-12 w-full md:w-auto pl-16 md:pl-0">
                    <div className="font-space-mono text-xs text-dark-ink/60 max-w-xs line-clamp-3">
                      {post.description}
                    </div>
                  </div>
                </Link>
          ...
          <ScrollReveal delay={0.4} className="mt-20">
            <Link
              href="/blog"
              className="inline-flex items-center font-space-mono text-xs uppercase tracking-widest text-rust hover:text-dark-ink transition-colors focus-visible:ring-2 focus-visible:ring-rust focus-visible:outline-none p-1 rounded-sm"
            >
